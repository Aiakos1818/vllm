...
def _resolve_gdn_prefill_backend(
    ...
    * Blackwell (SM10.x) with ``head_k_dim == 128``;
    ...
    # Local SM70/SM75 (2080 Ti) backend: the FlashQLA legacy GDN prefill kernel.
    # Neither FlashInfer nor CuteDSL supports Turing, so this is the only
    # accelerated prefill path on this hardware.
...
def flashqla_legacy_chunk_gated_delta_rule(
    ...
    Turing has no FlashInfer/CuteDSL GDN prefill kernel; this wraps the
    FlashQLA legacy (SM70/SM75) implementation instead.
    """
    ...
    def run_one(
        start: int | None = None, end: int | None = None, state_idx: int = 0
    ):
